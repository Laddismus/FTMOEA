...
def main() -> None:
    parser = argparse.ArgumentParser(description="ExitAgent training CLI")
    parser.add_argument("--env-config", default="configs/rl/env.yaml", help="Path to env config.")
    parser.add_argument("--agent-config", default="configs/rl/exit_agent.yaml", help="Path to exit agent config.")
    parser.add_argument("--output-dir", default="models/exit_agent/exp001", help="Directory to store checkpoints.")
    args = parser.parse_args()
    logging.basicConfig(level=logging.INFO)

    env = _build_env(args.env_config)
    obs_spec = RLObsSpec(shape=env.obs_spec.shape, dtype="float32", as_dict=False)
    action_spec = ActionSpec(action_type="discrete", num_actions=6)

    agent_cfg = ExitAgentConfig(**yaml.safe_load(Path(args.agent_config).read_text()))
    agent = ExitAgent(agent_cfg, obs_spec=obs_spec, action_spec=action_spec)
    train_cfg = ExitTrainConfig()
    summary = train_exit_agent(env, agent, train_cfg, checkpoint_dir=Path(args.output_dir))
    logger.info("ExitAgent training complete | mean_return=%.4f | best_return=%.4f", summary.mean_return, summary.best_return)
