...
def train_risk_agent(env: RLTradingEnv, agent: RiskAgent, train_config: TrainLoopConfig, checkpoint_dir: Path | None = None) -> TrainSummary:
    replay = ReplayBuffer(capacity=train_config.replay_capacity, obs_dim=agent.obs_dim)
    episode_returns: List[float] = []
    checkpoint_dir = checkpoint_dir or Path("models/risk_agent")
    ...
    for ep in range(train_config.total_episodes):
        ...
        if (ep + 1) % train_config.log_interval == 0:
            logger.info("Episode %d/%d | return=%.4f", ep + 1, train_config.total_episodes, ep_return)
